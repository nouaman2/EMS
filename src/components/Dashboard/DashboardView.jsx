import React, { useState, useEffect } from 'react';
import { useParams } from 'react-router-dom';
import FeedChart from '../Chart/FeedChart';
import { getDashboardTypeData } from '../../services/emonAPI';

const DashboardView = ({isDarkMode}) => {
  const { type } = useParams(); // Get the dynamic "type" parameter from the URL
  const [loading, setLoading] = useState(true); // Loading state
  const [error, setError] = useState(null); // Error state
  const [chartData, setChartData] = useState(null); // Chart data state

  console.log('DashboardView type:', type); // Log the type parameter for debugging
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        setLoading(true);
        const data = await getDashboardTypeData(type); // Fetch data using the "type" parameter
        setChartData(data); // Update the chart data
        setLoading(false); // Stop loading
      } catch (err) {
        setError(`Failed to load data for dashboard: ${type}`); // Set error message
        setLoading(false); // Stop loading
        console.error('Error fetching dashboard data:', err);
      }
    };

    fetchDashboardData(); // Call the function to fetch data
  }, [type]); // Re-run the effect when the "type" parameter changes

  if (loading) return <div className="loading">Loading dashboard...</div>;
  if (error) return <div className="error">{error}</div>;

  return (
    <div className="dashboard-view">
      <h2 className="dashboard-title">{type}</h2> {/* Display the dashboard name */}
      <div className="chart-container">
        <FeedChart
          data={chartData.datasets.map((d) => ({
            label: d.label,
            data: d.data,
          }))}
          feedName={type} // Use the "type" as the feed name
          isDarkMode={isDarkMode} // Pass the dark mode state to the chart
        />
      </div>
    </div>
  );
};

export default DashboardView;